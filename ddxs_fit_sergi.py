...
def miniboone_chisq(L):
    dof = 137-1
    data = Miniboone_ddxs.ravel()
    model = make_double_diff_sergi(L)
    model = model.ravel()
    err = Miniboone_Error.ravel()
    chisq_list = sq((data-model)/err)
    chisq_dof = sum(chisq_list)/dof
    print chisq_dof
...
double_diff_miniboone = make_double_diff_sergi(res_miniboone.x[0])

#print(" M_A_miniboone = %s" % res_miniboone.x[0])
print(" Lambda_miniboone = %s" % res_miniboone.x[0])

M_A = 1.35
#M_A_miniboone = res_miniboone.x[0]
Lambda_miniboone = res_miniboone.x[0]
if M_A == 1.35:
    col = 'red'
else:
    col = 'green'
...
length_miniboone  =  137
#length_total = len(total_ddxs_calc)

miniboone_chi_sq,miniboone_tot_chi_sq =  calc_chi_squared(double_diff_miniboone, Miniboone_ddxs, Miniboone_Error)
...
ax1.legend(loc=(0.6,0.4))

x,y = meshgrid(T_mu_1D,cos_mu_1D,indexing='ij')
ax1.scatter(x,y,double_diff_miniboone,color='orange',marker='s',label=r"$M_A = 1.35$ GeV, $\Lambda = %s$ GeV" % (round_sig(Lambda_miniboone)))
ax1.scatter(x,y,Miniboone_ddxs,color='black',marker='s',label="Minerva Neutrino Data",depthshade=False)
ax1.legend(loc=(0.35,0.65))
figax1.savefig("Desktop/Research/Axial FF/Plots/miniboone_ddxs_sergi_fit.pdf" )
